...
class ChatRepository:
    ...
    async def create_chat(self, chat_data: CreateChat) -> ChatOut:
        try:
            new_chat = Chat(**chat_data.dict(exclude={"user_uuids"}))
            self.db.add(new_chat)
            await self.db.flush()
            self.db.add_all([
                                UserChat(user_uuid=user_uuid, chat=new_chat.id) for user_uuid in chat_data.user_uuids
                            ] + [UserChat(user_uuid=chat_data.creator_uuid, chat=new_chat.id)])
